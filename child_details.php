    ...
    if (isset($_POST['register'])) {
        ...
        if ($password !== $confirm_password) {
            ...
            if (mysqli_num_rows($email_check_result) > 0) {
                ...
                $sql = "INSERT INTO `child`(`c_id`, `c_name`, `c_age`, `c_gender`, `c_dob`, `c_location`, `p_email`, `password`) 
                VALUES ('[value-1]','$name',' $age','  $gender','  $date','$location','  $email ',' $password ')";
    
                $result = mysqli_query($con, $sql);
    
                if ($result) {
                    echo "<script>alert('You are registered successfully');
                     window.location.href='login_parent.php';</script>";
                } else {
